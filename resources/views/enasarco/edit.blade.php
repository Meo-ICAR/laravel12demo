...
<div class="container mx-auto px-4 py-6">
                ...
                <ol class="flex items-center space-x-4">
                    ...
                    <li>
                        <div class="flex items-center">
                            ...
                            <span class="ml-4 text-sm font-medium text-gray-500">Modifica Record</span>
                        </div>
                    </li>
                </ol>
            </nav>
            <h2 class="mt-2 text-2xl font-bold leading-7 text-gray-900 sm:text-3xl sm:truncate">Modifica Record ENASARCO</h2>
            <p class="mt-1 text-sm text-gray-500">Modifica i dettagli del record ENASARCO.</p>
        </div>
    ...
    <div class="bg-white shadow overflow-hidden sm:rounded-lg">
        <form action="{{ route('enasarco.update', $enasarco->id) }}" method="POST" class="space-y-8 divide-y divide-gray-200">
            @csrf
            @method('PUT')
                    ...
                    <div class="space-y-6 sm:space-y-5">
                            ...
                            <div class="mt-1 sm:mt-0 sm:col-span-2">
                                <select id="enasarco" name="enasarco" class="max-w-lg block w-full shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:max-w-xs sm:text-sm border-gray-300 rounded-md @error('enasarco') border-red-300 text-red-900 placeholder-red-300 focus:outline-none focus:ring-red-500 focus:border-red-500 @enderror" required>
                                    @foreach($enasarcoTypes as $type)
                                        <option value="{{ $type }}" {{ old('enasarco', $enasarco->enasarco) == $type ? 'selected' : '' }}>{{ ucfirst($type) }}</option>
                                    @endforeach
                                </select>
                                @error('enasarco')
                        ...
                        <div class="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5 px-6 py-5">
                            ...
                            <div class="mt-1 sm:mt-0 sm:col-span-2">
                                <input type="number" name="competenza" id="competenza" min="2000" max="{{ $currentYear + 2 }}"
                                       value="{{ old('competenza', $enasarco->competenza) }}"
                        ...
                        <div class="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5 px-6 py-5">
                            <div>
                                ...
                                <p class="mt-1 max-w-2xl text-sm text-gray-500">Modifica gli importi in euro (€)</p>
                            </div>
                                    ...
                                    <div>
                                        ...
                                        <div class="mt-1 relative rounded-md shadow-sm">
                                            ...
                                            <input type="number" name="minimo" id="minimo" step="0.01" min="0"
                                                   value="{{ old('minimo', number_format($enasarco->minimo, 2, '.', '')) }}"
                                    ...
                                    <div>
                                        ...
                                        <div class="mt-1 relative rounded-md shadow-sm">
                                            ...
                                            <input type="number" name="massimo" id="massimo" step="0.01" min="0"
                                                   value="{{ old('massimo', number_format($enasarco->massimo, 2, '.', '')) }}"
                                    ...
                                    <div>
                                        ...
                                        <div class="mt-1 relative rounded-md shadow-sm">
                                            ...
                                            <input type="number" name="minimale" id="minimale" step="0.01" min="0"
                                                   value="{{ old('minimale', number_format($enasarco->minimale, 2, '.', '')) }}"
                                    ...
                                    <div>
                                        ...
                                        <div class="mt-1 relative rounded-md shadow-sm">
                                            ...
                                            <input type="number" name="massimale" id="massimale" step="0.01" min="0"
                                                   value="{{ old('massimale', number_format($enasarco->massimale, 2, '.', '')) }}"
            ...
            <div class="pt-5">
                <div class="flex justify-between px-6 py-4 bg-gray-50">
                    <div>
                        <form action="{{ route('enasarco.destroy', $enasarco->id) }}" method="POST" onsubmit="return confirm('Sei sicuro di voler eliminare questo record? Questa azione non può essere annullata.');">
                            @csrf
                            @method('DELETE')
