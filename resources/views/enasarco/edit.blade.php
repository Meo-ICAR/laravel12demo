...
<div class="container mx-auto px-4 py-6">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold text-gray-800">Modifica Record ENASARCO</h1>
    ...
    <div class="bg-white shadow-md rounded-lg p-6">
        <form action="{{ route('enasarco.update', $enasarco->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <select name="enasarco" id="enasarco" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                        @foreach($enasarcoTypes as $type)
                            <option value="{{ $type }}" {{ $enasarco->enasarco == $type ? 'selected' : '' }}>{{ ucfirst($type) }}</option>
                        @endforeach
                    </select>
                    @error('enasarco')
                ...
                <div>
                    ...
                    <input type="number" name="competenza" id="competenza" min="2000" max="{{ date('Y') + 1 }}" 
                ...
                <div>
                    ...
                    <input type="number" name="minimo" id="minimo" step="0.01" min="0" 
                           value="{{ old('minimo', $enasarco->minimo) }}" 
                           class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                    @error('minimo')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="number" name="massimo" id="massimo" step="0.01" min="0" 
                           value="{{ old('massimo', $enasarco->massimo) }}" 
                           class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                    @error('massimo')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="number" name="minimale" id="minimale" step="0.01" min="0" 
                           value="{{ old('minimale', $enasarco->minimale) }}" 
                           class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                    @error('minimale')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="number" name="massimale" id="massimale" step="0.01" min="0" 
                           value="{{ old('massimale', $enasarco->massimale) }}" 
                           class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                    @error('massimale')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
